...
def test_elements_methods_exist(ts):
    """Checks that barycentric, geocentric, and geometric positions have an 
    elements method
    """
    t = ts.utc(2015, 3, 2, 12)
    barycentric = earth.at(t)
    geocentric = (moon - earth).at(t)
    geometric = (earth - sun).at(t)
    for item in barycentric, geometric, geocentric:
        assert hasattr(item, 'elements')


def test_single_time(ts):
    """Tests creation of an OsculatingElements object with a single set of elements
    """
    elements = (moon-earth).at(ts.utc(2015, 3, 2, 12)).elements()
...
def test_multiple_times(ts):
    ...
    elements = (moon-earth).at(time).elements()
